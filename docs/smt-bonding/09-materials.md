...
| 黏度（Viscosity） | 影響印刷性；數值依量測法而異 | Malcom 螺旋式：150–250 Pa·s |
...
| 粒子直徑 | 3–5 μm（顯示器應用常見），需小於端子間距的 1/3 |
...
```mermaid
flowchart TD
    DS["Datasheet"] --> A["製程視窗<br/>（Process Window）<br/>溫度×壓力×時間可操作範圍"]
    DS --> B["儲存與有效期<br/>開封後壽命縮短"]
    DS --> C["測試方法<br/>廠商自測 vs 客戶端測試條件"]
    DS --> D["失效模式<br/>超出視窗的後果"]
```
