...
| 方法 | 固定什麼 | 觀察什麼 | 適合用途 | 主要風險 |
|---|---|---|---|---|
| lower envelope | 多組訓練曲線的 FLOPs | 同 compute 下最低 loss 對應的模型大小 | 從已跑實驗回推出 compute-optimal trend | 受既有 sweep 覆蓋範圍限制 |
| IsoFLOPs | 多個 FLOP budget | 每個 budget 下模型大小與資料量的最佳 tradeoff | 直接回答固定 compute 時該放大模型還是資料 | 需要在每個 budget 下掃足夠設計點 |
| joint functional form | `N`、`D` 與 loss surface | 擬合後在固定 `C` 下求最佳配置 | 可做平滑外推與分析 | 形式假設錯誤會改變結論 |
...
- Course material：`data/cs336/lectures material/lecture_09.pdf`。狀態：已核對 PDF metadata / outline；投影片未完整閱讀。
- Assignment 關聯：Assignment 3（`data/cs336/code/assignment3-scaling-main/`）對應 training API、budget/experiment 查詢、IsoFLOPs / scaling law 建構、loss projection 與 final submission 的實作/分析範圍。狀態：已核對 README、PDF outline、API/test 檔案；handout 未完整閱讀。
- 待補：投影片中的 exact equations、圖表、作者名校正、Chinchilla 三種方法的數值與 MoE/critical batch size 的圖示細節。
