...
```mermaid
flowchart LR
  A["HBM input tensor"] --> B["tl.load<br/>block tile"]
  B --> C["register / shared memory<br/>local compute"]
  C --> D["reduction / fusion<br/>accumulator"]
  D --> E["tl.store<br/>HBM output tensor"]
```
...
- Course material：`data/cs336/lectures material/lecture_06.py`；trace：`data/cs336/lectures material/var/traces/lecture_06.json`；PTX：`data/cs336/lectures material/var/triton_gelu-ptx.txt`。狀態：已核對 lecture README 與程式主流程；trace/PTX 未讀。
- Assignment 關聯：Assignment 2（`data/cs336/code/assignment2-systems-main/`）對應 profiling/benchmarking、single-GPU memory、FlashAttention-2、Triton kernel、DDP/FSDP 與 sharded optimizer 的實作範圍；本講主要銜接 GPU kernel 與 FlashAttention 基礎。狀態：已核對 README、PDF outline、測試介面；handout 未完整閱讀。
- 待補：XLA 相關本地材料未在 `lecture_06.py` 主流程核對出獨立 handout 或 PDF。
