...
- Course material：`data/cs336/lectures material/lecture_08.pdf`。狀態：已核對 PDF metadata / outline；投影片未完整閱讀。
- Assignment 關聯：Assignment 2（`data/cs336/code/assignment2-systems-main/`）對應 DDP、FSDP、optimizer state sharding、communication primitives、data/FSDP/tensor/2D parallelism analysis 的實作/分析範圍。狀態：已核對 README、PDF outline、測試介面；handout 未完整閱讀。
