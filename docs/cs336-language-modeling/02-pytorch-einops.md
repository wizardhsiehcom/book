...
| dtype | bytes | sign | exponent | mantissa | 訓練時的直覺 |
|---|---:|---:|---:|---:|---|
| FP32 | 4 | 1 | 8 | 23 | range 與 resolution 都較充足，但記憶體成本高 |
| FP16 | 2 | 1 | 5 | 10 | 省記憶體，但 exponent 少，較容易 overflow / underflow |
| BF16 | 2 | 1 | 8 | 7 | 保留 FP32 等級的 range，犧牲尾數精度 |
...
```mermaid
flowchart LR
  A["HBM / memory<br/>bytes moved"] --> B["accelerator<br/>FLOPs"]
  B --> C["HBM / memory<br/>write back"]
  A -. "頻寬限制" .-> C
  B -. "算力限制" .-> D["peak FLOPS/s"]
```
...
- Course material：`data/cs336/lectures material/lecture_02.py`；traces：`data/cs336/lectures material/var/traces/lecture_02.json`、`data/cs336/lectures material/var/traces/lecture_02_recording.json`。狀態：已核對 lecture README 與程式主流程；traces 未讀。
- Assignment 關聯：Assignment 1（`data/cs336/code/assignment1-basics-main/`）對應 PyTorch module、tensor shape、optimizer、data loader、checkpointing 與 training utilities 的實作範圍。狀態：已核對 README、PDF outline、測試介面；handout 未完整閱讀。
