...
厚度足夠只是重工的必要考量之一。以下是依前述流程整理的教學記錄表；實際權責與門檻須由委託規格決定。

| 處置出口 | 決定前需要的資料 | 目的與完成條件 |
|---|---|---|
| 暫停、隔離待查 | 片號／批號、缺漏履歷、異常位置、待補項目及負責人 | 保持可追溯；補足資料後重新評估，尚未放行加工 |
| 拒收或退回 | 不適用原因、來料條件、交接與所有權約定 | 阻止不適合的路徑；退回本身不等於報廢 |
| 複驗 | 原始結果、方法、校準、抽樣與規格版本 | 釐清判定是否有效；保留原始不合格結果，不反覆測到通過才停 |
| 重工 | 缺陷性質、可恢復依據、追加移除量、污染相容性與授權 | 改善指定異常；完成後重新驗證受影響品質，不能只驗修正的一項 |
| 改作其他用途 | 新用途要求、污染與幾何適配、接收方核准 | 建立新的使用資格；不能用較寬厚度要求掩蓋未知污染 |
| 終止原用途 | 不可恢復證據或無可接受路徑、工件歸屬 | 關閉原用途資格；後續退回、報廢或材料去向依約確認 |

**再生是一種恢復工件使用條件的加工；重工是針對不合格再做處理；複驗則先確認判定。** 三者可能出現在同一個生命週期，但不能把所有再生片都統計成「修好一片壞晶片」。逐案練習見[處置案例章](11-disposition-cases.md)。

公開業者案例可幫助理解這些出口：[NanoSILICON 的品質保證說明](https://nanosiliconinc.com/)寫明單片失敗會觸發整批 QC 複核，必要時再做全批重工。這證明重工可能是經檢查決定的路徑；它是該公司的公開作法，不能當成昇陽政策或無限重工的保證。[本輪來源 R4](appendix-sources.md#recovery-update)
