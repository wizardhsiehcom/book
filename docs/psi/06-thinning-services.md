...
背磨與拋光是移除材料的加工，因此「多磨掉的基材」不能靠再拋光長回原厚度。這是材料去除機制的推論；[DISCO 超薄研磨說明](https://www.disco.co.jp/eg/solution/library/grinder/thin.html)支持薄化、損傷與後續處理的背景，沒有提供昇陽的異常放行規則。

| 加工後情況 | 有條件的評估方向 | 停止條件 |
|---|---|---|
| 尚未到目標厚度 | 評估追加移除及重新量測 | 沒有可接受的表面、幾何、支撐或功能餘裕 |
| 已薄於原規格下限 | 停止按原規格交付，由權責方評估其他處置 | 不得以再拋光宣稱恢復厚度；變更規格須重新核准 |
| 表面損傷或形貌異常 | 先辨識損傷及所需移除量，再評估可恢復性 | 為去除異常而超出終厚度或功能限制 |
| 裂紋、正面功能疑慮 | 保留證據、界定受影響範圍與責任 | 未證明可交付前，不因尺寸通過就放行 |

表中是教學判斷方向，不能將每種異常直接等同整片報廢。元件晶圓有正面結構與累積價值；即使可以去掉表面某一層，也不代表原元件得救。再生監控片的轉用途評估，同樣不能直接移植到這種工件。
