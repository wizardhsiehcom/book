...
## 先把背景補齊：程序、容器物件和檔案來源會分開消失

一個 container 至少有三個值得分開看的部分：用來建立它的 image、容器自己的可寫層，以及可能接上的 bind mount 或 volume。主程序退出時，首先消失的是「正在跑的程序」；只要沒有使用 `--rm`，container 物件和可寫層仍可能留著。這也是為什麼 `docker exec` 進不去，卻不代表所有檔案都拿不回來。

反過來，`docker cp` 讀的是容器目前可見的檔案路徑，不是程序記憶體，也不會把外部資料源自動變成備份。結果檔若寫在可寫層，停止後仍有機會取出；若它在 volume、tmpfs 或已被 `--rm` 刪除，就要走不同的保存路徑。本章先用一個保留的失敗容器和一個 `--rm` 容器，把「停止」與「刪除」的差別固定下來。

```mermaid
flowchart LR
    R["docker run"] --> P["PID 1<br/>程序執行"]
    P -. "尚未退出" .-> E["docker exec<br/>可進入同一 container"]
    P --> X["程序退出"]
    X --> K["container 保留<br/>可 inspect / diff / cp"]
    X -. "--rm" .-> D["container 物件刪除<br/>不能再 cp"]
```

## 這一招其實在教什麼：把生命週期與證據保管分開

這章不是只教 `docker cp`，而是在建立一個和 C++ child process、暫存目錄及 CI artifact 相同的觀念：程序退出、執行物件消失、結果檔被保存，是三件不同的事。故障現場若沒有保管策略，第二次重跑很容易把第一個線索覆蓋掉。
...
若失敗產物對排障有價值，正式流程應明確定義保存哪些檔案、保留多久、由誰收集，並讓 CI 或 wrapper 在退出後自動打包最小證據。只有在確定不需要任何退出後資料時，才適合把 `--rm` 當成預設；清理本身不能取代 artifact policy。
