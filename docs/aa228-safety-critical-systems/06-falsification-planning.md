# 第 6 章：基於規劃的否證 (Falsification through Planning)

在上一章中，我們探討了如何將否證 (Falsification) 轉換為一個最佳化問題。然而，單純將「強健性 (Robustness)」最小化，往往會導致系統找到那些在現實中極不可能發生的邊緣失效情況。本章將介紹如何尋找**高機率的失效 (Likely Failures)**，並引入**樹狀搜尋 (Tree Search)** 與規劃演算法，透過逐步建立軌跡的方式來解決高維度空間搜尋的挑戰。

## 6.1 尋找高機率的失效

為了確保找到的失效是具有現實意義的，我們需要在目標函數中加入軌跡的「可能性 (Likelihood)」。一條軌跡的可能性是由初始狀態的機率，以及後續每一步擾動 (Disturbance) 的機率相乘而來。在實作上，為了避免連續相乘導致數值下溢 (Numerical Underflow)，我們通常採用**對數機率密度 (Log PDF)** 並將其相加。

在最佳化時，我們可以將目標函數設計為兩者的權衡：
1. **若軌跡尚未失效**：最小化強健性 (朝失效邊界靠近)。
2. **若軌跡已失效**：最小化負對數機率 (Negative Log-Likelihood)，使該軌跡變得更可能發生。

實務上，我們常使用一個權重參數 $\lambda$ 來平衡強健性與對數機率。
...
## 6.2 樹狀搜尋框架 (Tree Search Framework)
...
## 6.4 蒙地卡羅樹狀搜尋 (Monte Carlo Tree Search, MCTS)
...
當我們最終擴展了一個新節點，我們會使用隨機推演 (Rollout) 等方法來初始化其 $Q$ 值，並將這個新的價值與訪問次數 $N$ 沿著樹往回**傳播 (Propagate)**，藉此更新整棵樹的價值預估。這種機制確保了 MCTS 能夠在龐大且連續的搜尋空間中，有效且可靠地找到高機率的系統失效軌跡。

## 6.5 本章小結

- 為了找到**具有現實意義的失效**，目標函數須同時考量強健性與軌跡的對數概似度，並以權重 $\lambda$ 平衡兩者。
- 最佳化演算法的選擇取決於系統資訊：白箱可用**局部下降法**（依賴梯度），黑箱則用**零階方法**；**群體方法**有助於跳脫局部最佳解、發掘多種失效模式。
- 一次最佳化整條高維擾動序列代價高昂；**規劃**改為逐步建構軌跡，所有樹狀搜尋皆可歸納為「選擇—擴展」的循環。
- **RRT** 以隨機目標引導樹的生長，可藉「只從失效區域取樣目標」與「擾動最佳化」加速否證；引入可容許啟發式函數後即為 **A\* 搜尋**，可保證找到最佳路徑（離散空間）。
- **MCTS** 以訪問次數 $N$ 與價值估計 $Q$ 平衡探索與利用，搭配**漸進式擴展**與 **LCB** 處理連續擾動空間，能可靠地找到高機率失效軌跡。

下一章是客座講座：Anthony Corso 將介紹**自適應壓力測試（Adaptive Stress Testing, AST）**——以 MCTS 與強化學習為搜尋引擎、以「最可能的失效」為目標，正是本章規劃式否證在真實世界系統中的實務延伸。
