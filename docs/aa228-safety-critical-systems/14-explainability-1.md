# 第 14 章：可解釋性（一）(Explainability 1)

## 14.1 為什麼需要可解釋性？

在第 11 章到第 13 章中，我們以可達性分析為系統提供了形式化保證：只要可達集合與失效集合不相交，就能在給定假設下證明系統不會失效。加上更早的否證（第 5–6 章）與失效分佈與機率估計（第 8–10 章），我們已經能相當全面地回答「系統有沒有問題」。但這些方法本質上只回答了「它能不能運作」，而沒有回答「**它為何能運作**」或「**它為何會失效**」。
...
本章對應教科書《Algorithms for Validation》第 11 章，將介紹五大類可解釋性技術：**策略視覺化**、**特徵重要性**（含靈敏度分析與 Shapley 值）、**代理模型**、**反事實分析**，以及**失效模式特徵化**。
...
想像一個 5×5 的格子世界，每個格子可能在燃燒（紅色）或未燃燒（白色）。我們關心右上角格子在下一時步被點燃的機率。轉移規則是：若相鄰格子在燃燒，則有 30% 的機率被點燃。
...
**計算挑戰**：求和的範圍是「其餘特徵的所有子集」，數量隨特徵數指數成長。以野火例子來說，對單一格子計算 Shapley 值，需要對其餘 24 個格子的 $2^{24} \approx 1{,}600$ 萬種子集求和，難以窮舉。

**實際做法**：隨機抽取部分子集進行蒙地卡羅估計，通常可得到合理近似。

**倒立擺應用結果**：Shapley 值最高的擾動，是那些讓擺桿誤以為自己更接近直立的觀測擾動。將前四高 Shapley 值的擾動歸零後，原本的失效軌跡轉變為安全軌跡。
...
反事實分析問的是：「**如果輸入不是 $x$ 而是 $x'$，輸出會有何不同？**」更精確地說，它尋找的是**能使結果翻轉的最小輸入改動**——在所有會改變系統決策（或使失效軌跡變為安全）的替代輸入中，找出與原始輸入差異最小的那一個。

### 14.5.1 為什麼「最小改動」有解釋力？

特徵重要性（14.3 節）回答的是「哪些特徵對輸出影響最大」，本質上是**描述性**的解釋；反事實分析則提供**可操作**的解釋：「只要這裡不一樣，結果就會不同」。對工程師而言，這往往更直接有用——它不僅指出關鍵因素，還告訴我們需要改變多少才足以改變結局。

概念上，反事實分析可以表述為一個帶約束的最佳化問題：

$$\min_{\mathbf{x}'} \; d(\mathbf{x}, \mathbf{x}') \quad \text{s.t.} \quad \text{結果}(\mathbf{x}') \neq \text{結果}(\mathbf{x})$$

其中 $d(\cdot,\cdot)$ 是輸入空間上的距離度量。一個好的反事實解釋通常要同時滿足：

- **有效性（Validity）**：改動後結果確實翻轉（例如失效軌跡變為安全）。
- **最小性（Proximity）**：改動幅度越小，解釋越聚焦——「差一點點就不會失效」比「換一個完全不同的場景就不會失效」有意義得多。
- **合理性（Plausibility）**：改動後的輸入仍需物理上可行、落在真實資料分佈之內，否則解釋沒有工程價值。

```mermaid
flowchart LR
    A["原始輸入 x<br/>（導致失效）"] --> B["搜尋最小改動 δ"]
    B --> C["反事實輸入 x' = x + δ<br/>（結果翻轉：安全）"]
    C --> D["解讀 δ：<br/>哪些改動是關鍵？"]
```

### 14.5.2 與失效歸因的關係

反事實分析與本章其他方法有著自然的銜接。回顧 14.3.3 節倒立擺的結果：「將前四高 Shapley 值的擾動歸零後，失效軌跡轉變為安全軌跡」——這本身就是一個反事實陳述：*如果那幾個時間步的觀測擾動不曾發生，系統就不會失效*。換句話說，Shapley 值等特徵歸因方法找出「嫌疑犯」，反事實分析則進一步驗證「若沒有它，結局是否真的不同」，兩者結合能對個別失效給出更完整的因果敘事。

反事實分析也與否證（第 5–6 章）互為鏡像：否證從標稱行為出發，尋找**使系統失效**的擾動；失效歸因語境下的反事實分析則從失效軌跡出發，尋找**使結果變回安全**的最小改動。兩者都可以用最佳化的框架求解。

具體的求解演算法（如何選擇距離度量、如何在連續與離散輸入上搜尋反事實），教科書《Algorithms for Validation》第 11 章有詳細介紹，此處僅作概念性說明。

---

## 14.6 失效模式特徵化 (Failure Mode Characterization)

在我們透過否證或失效分佈抽樣收集了大量失效軌跡後，如何讓這些失效資料對人類工程師有意義？答案是**聚類（Clustering）**。
...
**下一章預告**：本章的方法大多停留在「輸入—輸出」層次。下一章（第 15 章）是 2026 年班的補充講次，將以三個工程問題重新審視可解釋性，並從特徵歸因走向模型內部的**機制性解讀**（稀疏自編碼器與電路追蹤）。
