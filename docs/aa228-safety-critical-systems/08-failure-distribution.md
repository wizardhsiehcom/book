# 第 8 章：失效分佈 (Failure Distribution)

在第 5 章與第 6 章中，我們探討了**否證 (Falsification)**——分別透過最佳化與規劃找到系統的**一個**失效案例，也許是最可能發生的失效；第 7 章的客座講座則展示了這類方法在真實系統中的應用與挑戰。然而，有時候我們想要的不僅僅是一個失效範例，而是全面了解「所有可能的失效樣貌」。本章將介紹如何從**完整的失效分佈**中取樣，讓我們能夠回答：「系統失效時，究竟會呈現哪些不同的行為模式？」

## 8.1 失效分佈的數學定義

### 標稱軌跡分佈

我們用 $p(\tau)$ 表示系統在正常（標稱）操作條件下產生軌跡 $\tau$ 的機率密度，稱為**標稱軌跡分佈 (Nominal Trajectory Distribution)**。

### 失效分佈 (Failure Distribution)

**失效分佈**是在已知軌跡為失效的前提下，軌跡的條件機率：
...
| 部分 | 數學形式 | 意義 |
|------|----------|------|
| 分子 | $\mathbf{1}[\tau \notin \psi]\, p(\tau)$ | 決定分佈的**形狀**：失效軌跡保留其標稱機率，非失效軌跡機率設為零 |
...
2. 計算 $\tau$ 在標稱分佈下的似然度 $p(\tau)$

關鍵洞見：未正規化密度已足以讓我們**從分佈中取樣**！接下來介紹兩種方法。

## 8.3 拒絕取樣法 (Rejection Sampling)
...
```mermaid
flowchart TD
    A["1. 在靶板上繪製目標密度形狀"] --> B["2. 均勻投擲飛鏢<br/>（從提案分佈 q(τ) 取樣，再取樣高度 h）"]
    B --> C["3. 找出落在目標密度曲線<br/>以上的飛鏢"]
    C --> D["拒絕 (Reject)：落在曲線以上的飛鏢"]
    C --> E["保留 (Accept)：落在曲線以下的飛鏢"]
    E --> F["4. 將飛鏢投影到水平軸<br/>得到目標分佈的樣本"]
```
...
**前提**：需選定提案分佈 $q(\tau)$ 及常數 $c$，使得對所有 $\tau$：

$$c \cdot q(\tau) \geq \bar{p}(\tau)$$

```
演算法：拒絕取樣
輸入：目標密度 p̄, 提案分佈 q, 常數 c, 樣本數 N
輸出：近似服從目標分佈的樣本集合
...
**最簡單的選擇**：以標稱分佈作為提案分佈，$q(\tau) = p(\tau)$，$c = 1$。
...
**解決方案**：將提案分佈的均值移向失效區域附近，並盡量縮小 $c$（但不違反覆蓋條件）。
...
1. **難以設計合適的提案分佈**：在高維度系統（如單擺）中，往往無法直觀找到好的 $q(\tau)$。
2. **難以選定合適的 $c$**：若 $c$ 太大，大量樣本被拒絕；若 $c$ 太小，則違反覆蓋條件，得到錯誤的分佈。
3. **高維度下極度低效**：詳見教科書 §6.1 的單擺範例。

## 8.4 馬可夫鏈蒙地卡羅 (Markov Chain Monte Carlo, MCMC)
...
每個樣本 $\tau_{k+1}$ 依賴於前一個樣本 $\tau_k$。在無限多個樣本的極限下，鏈中樣本的分佈收斂到目標分佈。
...
| 情況 | 接受機率 | 解釋 |
|------|----------|------|
| $\bar{p}(\tau') > \bar{p}(\tau)$ | $\alpha = 1$（必然接受） | 新樣本**更可能**來自目標分佈，必然接受 |
| $\bar{p}(\tau') < \bar{p}(\tau)$ | $\alpha = \bar{p}(\tau')/\bar{p}(\tau) < 1$ | 新樣本較不可能，但仍有一定機率接受（探索低密度區域） |
| $\bar{p}(\tau') = 0$ | $\alpha = 0$（必然拒絕） | 新樣本完全不在目標分佈中，直接拒絕 |

這個規則確保了：採樣頻率在高密度區域更高，但也會偶爾探索低密度區域——最終整條鏈的樣本分佈收斂至目標分佈。
...
> 上方虛擬碼為對稱核函數的特例；以下 Julia 實作採用一般（非對稱核函數）的 Metropolis-Hastings 接受率 $\bar{p}(\tau')\,g(\tau \mid \tau') \,/\, \bar{p}(\tau)\,g(\tau' \mid \tau)$，當核函數對稱時兩者等價。
...
其中 $\rho(\tau)$ 是軌跡的**強健性值 (Robustness)**。

- $\Delta(\tau) = 0$：軌跡為失效（強健性值 $\leq 0$）
...
**等價理解**：此流程等同於以平滑 MCMC 鏈為提案分佈的**拒絕取樣**。
...
|------|------------|----------|------|------|
| **直接蒙地卡羅** | $q = p$（標稱分佈） | 軌跡為失效 | 實作簡單 | 稀有失效時極度低效 |
...
- **失效分佈**是在已知軌跡為失效條件下的軌跡分佈，其正規化常數難以計算。
- **未正規化失效密度**可以輕鬆計算，且足以讓我們取樣。
- **拒絕取樣**：用提案分佈取樣，按比例接受——但對稀有失效效率低落。
- **MCMC（Metropolis-Hastings）**：維護馬可夫鏈，在無限樣本極限下收斂到目標分佈；需要預熱捨棄與稀釋。
- **平滑化**：用距離函數替換硬指示函數，使 MCMC 能夠跨越多個失效模式。

本章刻意繞開了失效分佈的正規化常數——它正是**失效機率** $p_{\text{fail}}$。下一章（第 9 章）將正面處理這個量的估計問題，並介紹核心工具：**重要性採樣 (Importance Sampling)**。
