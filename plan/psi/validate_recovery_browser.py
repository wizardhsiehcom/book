...
OUT = ROOT / 'data/psi-recovery/validation'
...
names = sorted(p.name for p in (ROOT / 'book/psi/html').glob('*.html') if p.name != '404.html')
...
try:
    with sync_playwright() as p:
        ...
        for mode, viewport in [('desktop', {'width': 1440, 'height': 1000}), ('mobile', {'width': 390, 'height': 844})]:
            ...
            for name in names:
                source_name = 'README.md' if name == 'index.html' else Path(name).with_suffix('.md').name
                expected_diagrams = (ROOT / 'docs/psi' / source_name).read_text(encoding='utf-8').count('```mermaid')
                page = context.new_page()
                errors, warnings = [], []
                page.on('pageerror', lambda e: errors.append(str(e)))
                page.on('console', lambda m: warnings.append(m.text) if m.type == 'warning' else None)
                page.goto(f'http://127.0.0.1:{server.server_port}/book/psi/html/{name}', wait_until='networkidle')
                ...
                if data['overflow'] or not data['footer'] or errors or len(data['diagrams']) != expected_diagrams or any(d['errors'] or not d['visible'] for d in data['diagrams']):
                ...
                if name in ('index.html', '04-reuse-economics.html', '06-thinning-services.html', '09-psi-evidence-map.html', '11-disposition-cases.html', '00-map.html'):
...
(ROOT / 'plan/psi/recovery-browser-validation.json').write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding='utf-8')
print(json.dumps(report['summary']))
print(json.dumps(report['small_diagram_labels']))

raise SystemExit(bool(report["failures"] or report["small_diagram_labels"]))
