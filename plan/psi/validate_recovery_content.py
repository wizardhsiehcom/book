"""Validate generated PSI links and record diagram provenance and source hashes."""
...
manifest = {'book': 'psi', 'checked_date': '2026-09-16',
            'image_type': 'original Mermaid diagrams and tables; existing photos and SVG retained',
            'external_images': 'unchanged; see docs/psi/99-image-credits.md and plan/psi/image-additions.json', 'pages': []}
for path in sorted(DOCS.glob('*.md')):
    ...
    manifest['pages'].append({'file': str(path.relative_to(ROOT)).replace('\\', '/'),
                              ...
                              'diagrams': diagrams})
(ROOT / 'plan/psi/recovery-image-manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding='utf-8')
...
(ROOT / 'plan/psi/recovery-content-validation.json').write_text(json.dumps(result, indent=2), encoding='utf-8')
