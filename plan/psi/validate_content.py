...
manifest = {'book': 'psi', 'checked_date': '2026-09-14',
            'image_type': 'original Mermaid diagrams and teaching tables',
            'external_images': [], 'pages': []}
for path in sorted(DOCS.glob('*.md')):
    ...
    manifest['pages'].append({'file': str(path.relative_to(ROOT)).replace('\\', '/'),
                              ...
                              'diagrams': diagrams})
(ROOT / 'plan/psi/image-manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding='utf-8')
...
(ROOT / 'plan/psi/content-validation.json').write_text(json.dumps(result, indent=2), encoding='utf-8')
